...
class TestMenu(BaseTest):
    ...
    def test_positive_logout(self, driver, log_page, login, menu_page):
        ...
        with allure.step('check if we are on the login page and the login button appears'):
            assert log_page.get_url() == URLs.login_url, 'Wrong url'
    ...
    @pytest.mark.positive
    def test_positive_about_btn(self, driver, login, menu_page, about_page):
        with allure.step('find and click the burger menu'):
            menu_page.menu_btn().click()
        ...
        with allure.step('check the expected url and the title'):
            ...
            assert about_page.get_url() == URLs.about_url and \
    ...
    def test_reset_app_state_negative(self, driver, login, inv_page, cart_page, menu_page):
