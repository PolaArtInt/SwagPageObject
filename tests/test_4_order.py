...
class TestOrder(BaseTest):
    ...
    @allure.title('processing purchase with relevant data')
    ...
    def test_positive_order(self, driver, fake, login, inv_page, cart_page, order_page):
        ...
        with allure.step('check the url and the success message presence'):
            assert order_page.get_url() == URLs.checkout_url and order_page.complete_msg, \
    ...
    @allure.epic('order module')
    @allure.feature('order')
    @allure.title('processing purchase with empty cart')
    @pytest.mark.defect
    @pytest.mark.xfail
    @pytest.mark.negative
    def test_negative_empty_order(self, driver, fake, login, inv_page, cart_page, order_page):
        with allure.step('go to the cart'):
            cart_page.cart_btn().click()
        ...
        with allure.step('expecting wrong url and no success message is provided'):
            try:
                assert order_page.get_url() != URLs.checkout_url and not order_page.complete_msg, \
