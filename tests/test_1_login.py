...
class TestLogin(BaseTest):
    ...
    def test_auth_positive(self, driver, inv_page, login):
        with allure.step('check the current url, the page header and the items presence on the inventory page'):
            assert inv_page.get_url() == URLs.inventory_url, 'Wrong url'
    ...
    @pytest.mark.positive
    def test_auth_positive_locked_out_user(self, driver, locked_out_log, log_page):
        with allure.step('check the current url and the error message is provided'):
            assert log_page.get_url() == URLs.url, 'Wrong url'
    ...
    @pytest.mark.positive
    def test_auth_positive_problem_user(self, driver, problem_log, inv_page):
        with allure.step('check the current url, the page header and the items presence on the inventory page'):
            assert inv_page.get_url() == URLs.inventory_url, 'Wrong url'
            assert inv_page.inventory_header().text == 'Products', 'Wrong page header'
            assert len(inv_page.inventory_cards()) > 0, 'There are no item cards on the inventory page'
    ...
    def test_problem_user_negative_inventory_imgs(self, driver, problem_log, inv_page):
    ...
    @pytest.mark.slow
    @pytest.mark.positive
    def test_auth_positive_performance_glitch_user(self, driver, glitch_log, inv_page):
    ...
    @pytest.mark.negative
    def test_auth_negative_wrong_login(self, driver, log_page):
        log_page.open()
